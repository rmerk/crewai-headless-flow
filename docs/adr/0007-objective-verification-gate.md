...
- **Known limitation (partially closed):** the finalize worker writes the ADR/report file *after* the last review-round verification. Operators who need that delta covered should set ``verify.pre_delivery_commands`` — the Flow runs those once in ``_maybe_deliver`` before push/pr and treats the result as the latest verification round for the delivery predicate.

State: `FlowState.verification_runs` records every round (revision-stamped); the debug report gains a `## Verification` section. Config: top-level `verify:` block (`commands`, `pre_delivery_commands`, `mode: gate|advisory`, `timeout`) and `deliver.remote`; per-run `--override-verify KEY=VALUE`. Cross-field rules: `deliver.pr` requires `deliver.push`, which requires `deliver.commit`.
