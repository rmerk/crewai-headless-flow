...
- **Hard guardrails.** Always a fresh branch (collision → `-2`, `-3`, …); refuse if the computed name is in `protected_branches`; stage only the flow's changed files with per-path `git add -- :(literal)<path>` — never `git add -A`, because preflight tolerates pre-existing operator dirt with a warning and delivery must not launder it into the flow's commit; unsafe (absolute/`..`/leading-`:`) paths are skipped and reported, not staged; no `--force`, no `reset`, no branch deletion anywhere. Detached and unborn HEAD are supported (`checkout -b` from unborn yields a root commit). *(Amended post-review: paths are staged as `:(literal)` pathspecs because changed-file names are worker-reported and untrusted — a name like `*.py` must not glob-expand into staging operator WIP. The same rule applies to `paths_policy.restore_denied_paths`, the platform's other git writer.)*
