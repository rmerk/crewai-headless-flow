...
Canonical topology lives in `config/flow.yaml` (`schema: crewai.flow/v1`, stage `execute_*` refs) and is bound at construction by `build_headless_flow` in `flow.py` (ADR-0012 Phase 3). Library/CLI entrypoints (`run_headless_flow` / `resume_headless_flow`) always use that builder. Decorator `@start` / `@listen` / `@router` wiring on `CrewAIHeadlessFlow` is retired; the class remains a shell for workers, HITL, verify, and thin stage wrappers.
