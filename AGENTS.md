...
| **Accepted** | **FlowDefinition topology cutover (hybrid)** | Move canonical start/listen/router wiring to `crewai.flow/v1` behind `call: code`; keep safety/HITL/verify/delivery in Python. Phase 0–2 landed (projection, `stages/` extraction, `config/flow.yaml` twin + kickoff equivalence); Phase 3 flips the CLI/library entrypoint. | Medium–High |
