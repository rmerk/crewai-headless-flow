...
| **Done** | **FlowDefinition topology cutover (hybrid)** | Move canonical start/listen/router wiring to `crewai.flow/v1` behind `call: code`; keep safety/HITL/verify/delivery in Python. Phase 0–3 landed (projection, `stages/` extraction, `config/flow.yaml` SoT, CLI/library entrypoint flip; decorators retired). | Medium–High |
